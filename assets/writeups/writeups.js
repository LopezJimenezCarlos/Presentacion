(function () {
  ...
  function setupMobileNav() {
    ...
    window.addEventListener("resize", () => { if (window.matchMedia("(min-width: 721px)").matches) closeMenu(); });
  }

  setupMobileNav();
  if (!writeups.length) return;

  const state = { query: "", category: "all", difficulty: "all" };
  ...
  function renderButtons(container, values, activeValue, onClick) {
    ...
    container.querySelectorAll("[data-value]").forEach((button) => button.addEventListener("click", () => onClick(button.dataset.value)));
  }
  ...
  function filteredItems() {
    return writeups.filter((item) => {
      const tags = Array.isArray(item.tags) ? item.tags : [];
      const stack = Array.isArray(item.stack) ? item.stack : [];
      const haystack = [item.title, item.summary, item.category, item.difficulty, tags.join(" "), stack.join(" ")].join(" ").toLowerCase();
  ...
  function renderCards() {
    ...
    if (els.resultCount) els.resultCount.textContent = `${items.length} ${items.length === 1 ? "writeup" : "writeups"}`;
  }
  ...
  function render() {
    renderButtons(els.filters, categories, state.category, (value) => { state.category = value; render(); });
    renderButtons(els.difficulty, difficulties, state.difficulty, (value) => { state.difficulty = value; render(); });
    renderCards();
  }
  ...
  if (els.input) {
    ...
    els.input.addEventListener("input", () => { state.query = els.input.value.trim().toLowerCase(); renderCards(); });
  }
